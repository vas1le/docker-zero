...
.PHONY: all build build-arm64 build-all fmt fmt-check lint test test-repeat race race-repeat vet vuln pycheck integration integration-retest doctor matrix-smoke matrix-harness-test compose-topology fuzz-smoke verify retest checksums package-test release hooks clean
...
checksums: build-all
	...
	python3 scripts/checksums_test.py dist

package-test: checksums

release: verify compose-topology package-test
