...
def create_and_start(socket_path: str, name: str, image: str, host_port: int | None = None, *, restart_policy: str = "no") -> str:
    body: dict[str, Any] = {"Image": image, "HostConfig": {"RestartPolicy": {"Name": restart_policy}}}
...
def run_seed(binary: str, seed: int) -> None:
    ...
    with RunningEngine(binary, seed=seed, container_endpoints="on") as engine:
        ...
        create_and_start(engine.socket_path, "nginx-zero", "nginx:alpine", engine.nginx_port, restart_policy="always" if seed == 2 else "no")
        create_and_start(engine.socket_path, "redis-zero", "redis:7-alpine", engine.redis_port, restart_policy="always" if seed == 2 else "no")
