...
func (api *DockerAPI) handleExecAction(w http.ResponseWriter, r *http.Request, rest string) {
	...
	case action == "start" && r.Method == http.MethodPost:
		api.startExec(w, r, exec)
	case action == "json" && r.Method == http.MethodGet:
		exec.mu.Lock()
		doc := map[string]any{
			"CanRemove": false, "DetachKeys": "", "ID": exec.ID, "Running": exec.Running,
			"ExitCode": exec.ExitCode, "ProcessConfig": map[string]any{"entrypoint": firstOrEmpty(exec.Command), "arguments": restOrEmpty(append([]string(nil), exec.Command...)), "privileged": false, "tty": false, "user": ""},
			"OpenStdin": false, "OpenStderr": exec.AttachStderr, "OpenStdout": exec.AttachStdout, "ContainerID": exec.ContainerID, "Pid": 0,
		}
		exec.mu.Unlock()
